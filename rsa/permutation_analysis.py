...
def mantel_test_feature_rdm(D_X: np.ndarray, feature: np.ndarray, B: int = 2000, metric: str = 'euclidean') -> dict:

    feature = np.asarray(feature)
    D_X = squareform(D_X)
    assert D_X.shape[0] == feature.shape[0] and D_X.shape[1] == D_X.shape[0], "D_X and feature must have same length... also D_X should be the RDM."
...
def run_mantel_test(D_X, X, subject_i, metadata, B: int = 2000, feature_selection: str = "centers") -> dict:
    ...
    for entry in metadata:
        ...
        # print(entry)
        # assert len(matches) == 1, print({f"{matches=}\n{len(matches)}"})
        match = matches[0]
