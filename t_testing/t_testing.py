...
def _load_ttest_results(config: Configuration, subject_id: int, shared_set: bool) -> np.ndarray:
    """
    ...
    if shared_set:
        ...
        t_results_dir = os.path.join(config.directories.t_test_results_dir, f"subj_{subject_id:02d}")
    
    t_results_filepath = os.path.join(t_results_dir, f"result_subj_{subject_id:02d}.npy")
...
def _load_subject_excel_data(
    ...
    processed_filenames = [
        e.split("/")[1].split(".")[0] 
        for e in excel_data["file_name"].tolist()
    ]
...
def _load_beta_arrays_for_file(
    ...
    if file_name in missing_subjects:
        if str(subject_id) in missing_subjects[file_name]:
            logging.info(f"Skipping {file_name} - missing in betas data for subject {subject_id}")
            return []
    ...
    if len(arrays) < n_samples:
        if is_shared_file:
            ...
            logging.warning(f"Insufficient samples for {file_name}: {len(arrays)} < {n_samples}")
            return []
...
def _filter_voxels_by_threshold(ttest_results: np.ndarray, t_threshold: float) -> np.ndarray:
    """
...
def _compute_outlier_scores_for_voxel(
    ...
    for sample_idx, value in enumerate(pos_values):
        ...
        results.append([
            voxel_index, "positive", sample_idx, 
            own_distance, other_distance, score
        ])
    ...
    for sample_idx, value in enumerate(neg_values):
        ...
        score = other_distance - own_distance
        results.append([
            voxel_index, "negative", sample_idx,
            own_distance, other_distance, score
        ])
...
def _save_outlier_results(
    ...
    if shared_set:
        ...
        results_dir = os.path.join(config.directories.t_test_results_dir, f"subj_{subject_id:02d}")
...
def _load_dataset_filenames(
    ...
    negative_filenames = [
        e.split("/")[1].split(".")[0] 
        for e in filtered_negative["file_name"].tolist()
    ]
    
    logging.info(f"Removed {len(negative_excel) - len(filtered_negative)} animate_faces samples")
...
def set_voxelwise_outlier_detection(
    ...
    for subject_id in tqdm(subjs_to_use, desc="Processing subjects"):
        logging.info(f"Performing voxelwise outlier detection for subject {subject_id:02d}")
        ...
        try:
            ...
            logging.info(f"Subject {subject_id:02d}: {len(selected_voxels)} voxels pass t > {t_threshold}")
            
            # Load dataset information
            _, pos_filenames = _load_subject_excel_data(config, subject_id, shared_set, 'face')
            _, neg_filenames = _load_subject_excel_data(config, subject_id, shared_set, 'non_face')
            
            # Load beta data for positive group
            pos_data = []
            for filename in pos_filenames:
                arrays = _load_beta_arrays_for_file(
                    config, filename, subject_id, n_samples=1, 
                    rng=random.Random(0), missing_subjects=missing_subjects
                )
            ...
            results_path = _save_outlier_results(config, subject_id, shared_set, results_array)
    ...
    for subject_id in tqdm(subjs_to_use, desc="Processing subjects"):
        ...
        try:
            ...
            positive_data = _load_beta_data_for_group(
                config, pos_filenames, subject_id, n_samples_per_file,
                rng, missing_subjects, shared_filenames
            )
            
            negative_data = _load_beta_data_for_group(
                config, neg_filenames, subject_id, n_samples_per_file,
                rng, missing_subjects, shared_filenames
            )
            ...
            # Save results
            results_path = _save_ttest_results(config, subject_id, shared_set, ttest_results)
