...
def sample_repeatability_distances_data(
    ...
    if os.path.exists(sample_repeatability_distances_pickle):
        logging.info(f"Loading existing data from {sample_repeatability_distances_pickle}")
        with open(sample_repeatability_distances_pickle, "rb") as f:
            results_all = pickle.load(f)
    else:
        logging.info(f"Generating new data, will save to {sample_repeatability_distances_pickle}")
            ...
            for distance_space in ["rdm", "mds"]:
                ...
                for stim in tqdm(np.unique(labels), leave=False):
                    ...
                    for b in range(n_perm):
                        ...
                        if perm_idx.size < 2:
                            raise RuntimeError("Permutation resulted in insufficient repeats")
    ...
    logging.info(f"Distance analysis complete. Results saved to {sample_repeatability_distances_excel}")
