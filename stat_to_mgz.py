        ...
        for hemi_i, hemi in enumerate(hemis):
            ...
            if clipping_value is None:
                data_out_file = os.path.join(
                    ...
                    sub,
                    "label",
                    f"{hemi}.t_test_{mode}_{str(threshold).replace('.', '_')}_label_{label}.mgz",
                )
            else:
                data_out_file = os.path.join(
                    config.freesurfer_dir,
                    sub,
                    "label",
                    f"{hemi}.t_test_{mode}_{str(threshold).replace('.', '_')}_label_{label}_clip_{clipping_value}.mgz",
                )
