...
def _construct_output_directory(config: Configuration, subdir: str = "mutually_exclusive") -> str:
    """
...
def _apply_mutual_exclusivity_filter(
    ...
    for index in tqdm(range(animate_nonanimate_results.shape[0]), desc="Applying mutual exclusivity"):
        if face_animate_results[index][0] > threshold:
            # Apply mutual exclusivity based on difference threshold
            if (face_animate_results[index][0] - animate_nonanimate_results[index][0]) < difference_threshold:
...
def _get_mask_file_path(config: Configuration, sub: str, hemi: str) -> str:
    ...
    mask_dir = os.path.join(
        config.nsd_data.stans_thesis_repo_data, 
        config.nsd_data.mask_data_dir
    )
...
def _construct_output_filename(
    ...
    if clipping_value is None:
        filename = f"{hemi}.t_test_{mode}_{threshold_str}_label_{label}_{shared_string}.mgz"
    else:
...
def mutually_exclusive_t_test_export(
    ...
    # Load t-test results
    face_animate_results = _load_t_test_results(config, subj_num, face_animate_t_test_subdir)
    animate_nonanimate_results = _load_t_test_results(config, subj_num, animate_nonanimate_t_test_subdir)
...
def t_test_results_to_mgz(
    ...
    assert mode in ["absolute", "signed"], f"Mode must be 'absolute' or 'signed', got '{mode}'"
    ...
    for sub_i, sub in enumerate(subs):
        logging.info(f"Processing {sub} - mode={mode}, thresholding={thresholding}, threshold={threshold}")
        ...
        for hemi_i, hemi in enumerate(hemis):
            ...
            data_out = _process_hemisphere_data(
                t_data, maskdata_long, hemi_i, hemisphere_shapes, 
                mode, threshold, thresholding, min_value
            )
            ...
            data_out = _apply_final_thresholding(
                data_out, t_data, maskdata_long, hemi_i, hemisphere_shapes,
