...
@st.cache_data
def load_face_detection_results_filenames(path: str, target_detections: int = 0) -> List[str]:
    """
    ...
    if defined_columns:
        for col in defined_columns:
            if col not in df_filtered.columns:
                default_val = 0 if col == 'cocoId' else ('unknown' if col == 'label' else pd.NA)
...
def main():
    ...
    subdir = "subj_08"  # TODO: Make this configurable
    ...
    images_per_page = st.sidebar.slider("Images per page", 1, 500, 26, key="imgs_per_page_slider")
    ...
    if dataset_mode == "faces":
        ...
        st.sidebar.info("Showing images with 1 face detection from `face_detection_result.json`.")
