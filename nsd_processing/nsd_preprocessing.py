...
def process_subject(subject):
    ...
    for session in tqdm(SESSIONS, desc=f"Subject {subject} sessions"):
        ...
        try:
            ...
            logging.debug(f"Processed and saved session {session} for subject {subject}")
            
        except Exception as e:
            logging.warning(f"Failed to process session {session} for subject {subject}: {e}")
            break
...
def concatenate_subject_sessions(subject):
    ...
    target_dir = os.path.join(data_dirs["target"], f"subj{subject:02d}")
    concatenated_file_path = os.path.join(target_dir, f"full_betas_subj{subject:02d}.npy")

    # Count available session files
    session_files = [f for f in os.listdir(target_dir) if f.startswith("betas_session_")]
    ...
    start_idx = 0
    for session in tqdm(range(1, available_n_sessions + 1), 
...
def example_usage():
    ...
    config.directories.image_betas_dir = "/media/harveylab/STORAGE1_NA/NSD/image_betas_standardize_no_shift"
    config.nsd_data.full_brain_data_dir = "/media/harveylab/STORAGE1_NA/NSD/full_brain_standardize_no_shift"
