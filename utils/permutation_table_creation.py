...
def color_p_values_log10_custom(
    s, vmin_log10=0, vmax_log10=5, cmap_name="YlGnBu", epsilon=1e-300
):
    """
...
for module in ["detection", "genderage"]:
    ...
    pivot = df_rdm.pivot_table(
        index="layer", columns="feature_selection", values=["r_obs", "p_value"]
    )
    ...
    idx = pd.IndexSlice
    styled = (
        pivot.style.format(precision=3)
        ...
        .apply(
            ...
            [
                {
                    "selector": "th, td",
                    "props": [("padding", "2px 4px"), ("font-size", "8pt")],
                }
            ]
        )
    )
    ...
    styled_borders = styled.set_table_styles(
        [
