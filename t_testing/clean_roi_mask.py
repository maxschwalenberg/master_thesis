...
def _load_hemisphere_masks(
    mask_path_lh: str, mask_path_rh: str
...
def _load_ttest_data(
    config: Configuration, subj_to_pick: int, subj_to_pick_shared: bool
) -> np.ndarray:
    """
...
def _apply_threshold_to_roi(
    mask: np.ndarray, t_data: np.ndarray, threshold: float, roi_label: int
) -> None:
    """
...
def modify_mask_with_ttest(
    ...
    _, mask_path_lh, mask_path_rh, new_mask_path_lh, new_mask_path_rh = (
        _construct_file_paths(
    ...
    logging.info(
        f"Successfully processed subject {subj_to_pick} with threshold {threshold}"
    )
