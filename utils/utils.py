...
def retrieve_stacked_betas(
    ...
    """
    assert train_mode in ("single", "multiple", "averaged"), f"Invalid train_mode: {train_mode}"
    
    logging.info(f"Loading beta coefficients - Subject: {subj}, Mode: {train_mode}, Test: {test}")
    ...
    if augment_shared_set:
        ...
        shared_df = pd.DataFrame(columns=temp.columns)  # Empty shared_df for later reference
        base_df = temp
    ...
    for mds_index, entry in enumerate(coco_ids):
        ...
        if not test and n_usable < 2:
            ...
            raise ValueError(f"Insufficient clean samples for image {entry} (need ≥2, got {n_usable})")
...
def load_negative_set(
    ...
    if filtered_count < initial_count:
        logging.info(f"Removed {initial_count - filtered_count} generic 'animate' labels")
    ...
    if remove_animate_face:
        ...
        if post_removal_count < pre_removal_count:
            logging.info(f"Removed {pre_removal_count - post_removal_count} animate_face labels")
